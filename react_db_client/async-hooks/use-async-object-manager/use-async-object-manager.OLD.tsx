...
export const useAsyncObjectManager = <DocType extends IDocument>({
  ...
  const combinedData: DocType = useMemo(() => {
    const _combinedData: DocType = merge(
      { ...(loadedData || ({} as Partial<DocType>)) },
      { ...(inputAdditionalData || ({} as Partial<DocType>)) },
      { ...editData },
      {
        uid,
        label:
  ...
  const deleteArgs = useMemo<[string, Uid]>(
    () => [collection, uid],
    [collection, uid]
  );
  ...
  const updateData = useCallback((newData) => {
    console.info('UPDATE ALL DATA', newData);
  ...
  const updateFormData = useCallback(
    (field, value, save = false, nested = false) => {
      console.info('UPDATE FIELD', field, value);
      setUnsavedChanges(true);
      setEditData((prev) => {

...
import { deepIsEqual, filterDataFunc } from '@react_db_client/helpers.filter-helpers';
...
const useDataManager = ({
  ...
  saveTotalsCallback = () => {},
  // eslint-disable-next-line no-unused-vars
  updatedDataHook = (row, column, field) => {},
  customFilters,
}) => {
  let error = null;
  const [intData, setIntData] = useState(dataIn);
  const [sortBy, setSortBy] = useState(() => sortByIn);
  const [unsavedChanges, setUnsavedChanges] = useState(false);
  // TODO: Implement trigger filter and sort
  // const [triggerFilter, setTriggerFilter] = useState(false);
  // const [triggerSort, setTriggerSort] = useState(false);

  const [intProcessedData, setIntProcessedData] = useState([]);
  ...
  useEffect(() => {
    if (intData) {
      ...
      if (recalculate && autoFilter && filters.length > 0) {
        updatedData = filterDataFunc(customFilters || {})(filters, updatedData);
      }
  ...
  if (!intProcessedData) error = 'Data is invalid';
  if (!headings) error = 'Input headings are invalid';
  ...
  const handleSaveData = () => {
    autoSaveCallback(intData, SAVE_ACTIONS.SAVE_BTN_CLICKED);
